...
import jp.nyatla.nyartoolkit.core.surfacetracking.featuremap.NyARSurfaceFeatureMap;
import jp.nyatla.nyartoolkit.core.surfacetracking.featuremap.NyARSurfaceFeatureMapUtils;
...
public class NyARSurfaceFeatureSetUtils
{
	...
	public static NyARNftFsetFile makeSurfaceFeature(INyARGrayscaleRaster[] i_rasters,double[] i_dpis)
    {
	...
	public static NyARNftFsetFile makeSurfaceFeature(INyARGrayscaleRaster[] i_raster,double[] i_dpi,
	    ...
	    double  max_sim_thresh, double  min_sim_thresh, double  sd_thresh)
	{
		...
		NyARNftFsetFile.NyAR2FeaturePoints[] list=new NyARNftFsetFile.NyAR2FeaturePoints[i_raster.length];
		for(int i2=0;i2<i_raster.length;i2++){
			//defocus 1
			INyARGrayscaleRaster src=i_raster[i2];
			INyARGrayscaleRaster imgBW1=NyARGrayscaleRaster.createInstance(src.getWidth(),src.getHeight());
			...
			//データ生成
	        list[i2]=new NyARNftFsetFile.NyAR2FeaturePoints(selected.getLength(),i2,maxdpi,mindpi);
	        NyARNftFsetFile.NyAR2FeaturePoints list_item=list[i2];
		...
		return new NyARNftFsetFile(list);
	}
}
