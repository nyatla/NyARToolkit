...
public class NyARSurfaceFeatureMapUtils
{
	...
	public static NyARSurfaceFeatureMap ar2GenFeatureMap(
        ...
        int ts1, int ts2,
        int search_size1,
        double  max_sim_thresh, double  sd_thresh )
	{
	...
	//	p = imgBW1;//ブラー1かいかけたやつっぽい
		...
		for(int j = 1; j < ysize-1; j++ ) {
			...
			{
				double dx=(	i_raster.getPixel(i+1,j-1)-i_raster.getPixel(i-1,j-1)
						+	i_raster.getPixel(i+1,j  )-i_raster.getPixel(i-1,j  )
						+	i_raster.getPixel(i+1,j+1)-i_raster.getPixel(i-1,j+1))/(double)(3*256);
				double dy=(	i_raster.getPixel(i+1,j+1)-i_raster.getPixel(i+1,j-1)
						+	i_raster.getPixel(i,  j+1)-i_raster.getPixel(i,  j-1)
						+	i_raster.getPixel(i-1,j+1)-i_raster.getPixel(i-1,j-1))/(double)(3*256);
...
//		}
	...
	/**
	 * 一時テンプレート。{@link NyARTemplatePatchImage}と同じだけど制度がdouble
	 */
	private static class TemplateImage
	{
		...
		public boolean make_template(INyARGrayscaleRaster imageBW,
            int cx, int cy,double  sd_thresh)
		{
			int      i, j;
			int ts1=this._ts1;
			int ts2=this._ts2;
	...
	//			ip = &imageBW[(cy+j)*xsize+(cx-ts1)];
				for( i = -ts1; i <= ts2 ; i++ ) {
					this.img[tp] = (double)(imageBW.getPixel(cx+i,cy+j)) - ave;
...
//		        ip = &imageBW[(cy+j)*xsize+(cx-ts1)];
		        for(int i = -ts1; i <= ts2 ; i++ ) {
		            double w2 = (double )(imageBW.getPixel(cx+i,cy+j)) - ave;
		            vlen2 += w2 * w2;
//		            w1 += *(tp++) * w2;
		            w1 += this.img[tp++]* w2;
		        }
