...
public class DoGScaleInvariantDetector {
    ...
    public DoGScaleInvariantDetector()
    {
         ...
         */
        public int width(){ return this.mWidth; }
...
//            }        	
         ...
         */
        public FeaturePointStack features(){ return this.mFeaturePoints; }
        
        /**
         * @return DoG Pyramid
         */
        public DoGPyramid dogPyramid(){ return this.mLaplacianPyramid; }
        ...
        private class BucketPair
        {
        	public float first;
        	public int second;
        }
        private class BucketStack extends NyARObjectStack<BucketPair>
        {
			protected BucketStack(int i_length) {
    	...
    	public static BucketStack[][] createBucketPairArray(int x,int y){
            ...
            float y)
        {
			...
			assert (int)Math.floor(x) == (int)x;//ASSERT((int)std::floor(x) == (int)x, "floor() and cast not the same");
			assert (int)Math.floor(y)==(int)y;//ASSERT((int)std::floor(y) == (int)y, "floor() and cast not the same");
			...
			// Some sanity checks
			assert yp >= 0 && yp < height;//ASSERT(yp >= 0 && yp < height, "yp out of bounds");
			assert yp_plus_1 >= 0 && yp_plus_1 < height;//ASSERT(yp_plus_1 >= 0 && yp_plus_1 < height, "yp_plus_1 out of bounds");
			assert xp >= 0 && xp < width;//ASSERT(xp >= 0 && xp < width, "xp out of bounds");
			assert xp_plus_1 >= 0 && xp_plus_1 < width;//ASSERT(xp_plus_1 >= 0 && xp_plus_1 < width, "xp_plus_1 out of bounds");
			...
			assert w0 >= 0 && w0 <= 1.0001; //ASSERT(w0 >= 0 && w0 <= 1.0001, "Out of range");
			assert w1 >= 0 && w1 <= 1.0001; //ASSERT(w1 >= 0 && w1 <= 1.0001, "Out of range");
			assert w2 >= 0 && w2 <= 1.0001; //ASSERT(w2 >= 0 && w2 <= 1.0001, "Out of range");
			assert w3 >= 0 && w3 <= 1.0001; //ASSERT(w3 >= 0 && w3 <= 1.0001, "Out of range");
			assert (w0+w1+w2+w3) <= 1.0001;//ASSERT((w0+w1+w2+w3) <= 1.0001, "Out of range");
         ...
         */
        private static void bilinear_upsample_point(
        	float[] xyp,
            float x,
            float y,
            int octave)
         ...
         */
        private void bilinear_downsample_point(float[] xyp,
                                              float x,
                                              float y,
                                              int octave) {
            float a, b;
        ...
        private void bilinear_downsample_point(float[] xysp,
                float x,
                float y,
                float s,
                int octave)
         ...
         */
        public void extractFeatures(GaussianScaleSpacePyramid pyramid,DoGPyramid laplacian)
        {
            ...
            for(int i = 1; i < mLaplacianPyramid.size()-1; i++) {
                ...
                if(im0.getWidth() == im1.getWidth() && im0.getWidth() == im2.getWidth()) { // All images are the same size
...
//                    ASSERT(im0.height() == im2.height(), "Height is inconsistent");
                    ...
                    for(int row = 1; row < heigh_minus_1; row++) {
                        ...
                        for(int col = 1; col < width_minus_1; col++) {
                            ...
                            if(
                            	value > im0b[im0_ym1+col-1]   &&         value > im0b[im0_ym1+col]     &&         value > im0b[im0_ym1+col+1]   &&         value > im0b[im0_y+col-1]     && 
                            	value > im0b[im0_y+col]       &&         value > im0b[im0_y+col+1]     &&         value > im0b[im0_yp1+col-1]   &&         value > im0b[im0_yp1+col]     && 
                            	value > im0b[im0_yp1+col+1]   && 
                    	        /* im1 - 8 evaluations */         
                            	value > im1b[im1_ym1+col-1]   &&         value > im1b[im1_ym1+col]     &&         value > im1b[im1_ym1+col+1]   &&         value > im1b[im1_y+col-1]     && 
                    	        value > im1b[im1_y+col+1]     &&         value > im1b[im1_yp1+col-1]   &&         value > im1b[im1_yp1+col]     &&         value > im1b[im1_yp1+col+1]   && 
                    	        /* im2 - 9 evaluations */          
                    	        value > im2b[im2_ym1+col-1]   &&         value > im2b[im2_ym1+col]     &&         value > im2b[im2_ym1+col+1]   &&         value > im2b[im2_y+col-1]     && 
                    	        value > im2b[im2_y+col]       &&         value > im2b[im2_y+col+1]     &&         value > im2b[im2_yp1+col-1]   &&         value > im2b[im2_yp1+col]     && 
                            ...
                            if(extrema) {
                                ...
                                fp.scale  = scale;
                                fp.score  = value;
                                fp.sigma  = (float) pyramid.effectiveSigma(octave, scale);
                                float[] tmp=new float[2];
                                bilinear_upsample_point(tmp,
                                                        col,
                                                        row,
                                                        octave);
                                fp.x=tmp[0];
                ...
                } else if(im0.getWidth() == im1.getWidth() && (im1.getWidth()>>1) == im2.getWidth()) { // 0,1 are the same size, 2 is half size
//                    ASSERT(im0.height() == im1.height(), "Height is inconsistent");
//                    ASSERT((im1.height()>>1) == im2.height(), "Height is inconsistent");
                        ...
                        for(int col = 2; col < end_x; col++) {
                            ...
                            if(
                   			 /* im0 - 9 evaluations */
								 ...
								 value > im0b[im0_ym1+col]	  && 	 value > im0b[im0_ym1+col+1]	&& 
								 value > im0b[im0_y+col-1]	  && 	 value > im0b[im0_y+col]		 && 
								 value > im0b[im0_y+col+1]	  && 	 value > im0b[im0_yp1+col-1]	&& 
								 value > im0b[im0_yp1+col]	  && 	 value > im0b[im0_yp1+col+1]	&& 
								 /* im1 - 8 evaluations */			
								 ...
								 /* im2 - 9 evaluations */			 
								 value > bilinear_interpolation(im2, ds_x-0.5f, ds_y-0.5f)	&& value > bilinear_interpolation(im2, ds_x,		ds_y-0.5f)	&&
								 value > bilinear_interpolation(im2, ds_x+0.5f, ds_y-0.5f)	&& value > bilinear_interpolation(im2, ds_x-0.5f, ds_y)		  &&
								 value > bilinear_interpolation(im2, ds_x,		ds_y)		&& value > bilinear_interpolation(im2, ds_x+0.5f, ds_y)		  &&
								 value > bilinear_interpolation(im2, ds_x-0.5f, ds_y+0.5f)	&& value > bilinear_interpolation(im2, ds_x,		ds_y+0.5f)	&&
                                ...
                                extrema = true;
                      			 ...
                      			 /* im0 - 9 evaluations */
                    			 value < im0b[im0_ym1+col-1]	&&
    							 value < im0b[im0_ym1+col]	&& 	 value < im0b[im0_ym1+col+1]	&& value < im0b[im0_y+col-1]	  && value < im0b[im0_y+col]		&&
    							 value < im0b[im0_y+col+1]	&& 	 value < im0b[im0_yp1+col-1]	&& value < im0b[im0_yp1+col]	  && value < im0b[im0_yp1+col+1]	&&
    							 /* im1 - 8 evaluations */			
    							 value < im1b[im1_ym1+col-1]	&& 	 value < im1b[im1_ym1+col]	&& value < im1b[im1_ym1+col+1] && value < im1b[im1_y+col-1]	&& 
    							 value < im1b[im1_y+col+1]	&& 	 value < im1b[im1_yp1+col-1]	&& value < im1b[im1_yp1+col]	  && value < im1b[im1_yp1+col+1]	&& 
    							 /* im2 - 9 evaluations */
    							 ...
    							 value < bilinear_interpolation(im2, ds_x-0.5f, ds_y+0.5f)	&& value < bilinear_interpolation(im2, ds_x,ds_y+0.5f)	&&
    							 value < bilinear_interpolation(im2, ds_x+0.5f, ds_y+0.5f)
                            ) { // if(NONMAX_CHECK(<, value)) { // strictly less than
                                extrema = true;
                            }
                ...
                } else if((im0.getWidth()>>1) == im1.getWidth() && (im0.getWidth()>>1) == im2.getWidth()) { // 0 is twice the size of 1 and 2
                    ...
                    for(int row = 1; row < height_minus_1; row++) {
                        ...
                        for(int col = 1; col < width_minus_1; col++) {
                            ...
                            if(
                                ...
                                value > im2b[im2_yp1+col+1]   &&
                                /* im2 - 9 evaluations */
                                value > bilinear_interpolation(im0, us_x-2.f, us_y-2.f)   && value > bilinear_interpolation(im0, us_x,     us_y-2.f)   &&
                                value > bilinear_interpolation(im0, us_x+2.f, us_y-2.f)   && value > bilinear_interpolation(im0, us_x-2.f, us_y)       &&
                                value > bilinear_interpolation(im0, us_x,     us_y)       && value > bilinear_interpolation(im0, us_x+2.f, us_y)       &&
                                value > bilinear_interpolation(im0, us_x-2.f, us_y+2.f)   && value > bilinear_interpolation(im0, us_x,     us_y+2.f)   &&
...
//                                mFeaturePoints.push_back(fp);
        ...
        boolean SolveSymmetricLinearSystem3x3(float[] x,float[] A, float[] b)
        {
        	...
        	m.m00=A[0];m.m01=A[1];m.m02=A[2];
        	m.m10=A[3];m.m11=A[4];m.m12=A[5];
        	m.m20=A[6];m.m21=A[7];m.m22=A[8];
        	if(!m.inverse(m)){
        		return false;
        	}
...
//            ASSERT(mBuckets[0].size() == mNumBucketsY, "Buckets are not allocated");
            ...
            assert mFeaturePoints.getLength() <= mMaxNumFeaturePoints;//, "Too many feature points");        	
...
//            ASSERT(mFeaturePoints.size() <= mMaxNumFeaturePoints, "Too many feature points");        	
        ...
        private void findFeatureOrientations(GaussianScaleSpacePyramid pyramid)
        {
            ...
            int num_angles;
            FeaturePoint[] mTmpOrientatedFeaturePoints=new FeaturePoint[mFeaturePoints.getLength()*kMaxNumOrientations];
            ...
            for(int i = 0; i < mFeaturePoints.getLength(); i++) {
                ...
                bilinear_downsample_point(
                	tmp,
					mFeaturePoints.getItem(i).x,
                ...
                x = ClipScalar(x, 0, pyramid.get(mFeaturePoints.getItem(i).octave, 0).getWidth()-1);
                y = ClipScalar(y, 0, pyramid.get(mFeaturePoints.getItem(i).octave, 0).getHeight()-1);
                
                FloatVector f=new FloatVector(mOrientations,0);                
                // Compute dominant orientations
                mOrientationAssignment.compute(
                	f,
                	mFeaturePoints.getItem(i).octave,
				   	mFeaturePoints.getItem(i).scale,
				   	x,
				   	y,
				   	s);
                num_angles=f.num;
...
//              int x,int y) 
        ...
        private boolean ComputeSubpixelHessian(
        		float[] H,float[] b,
        		KpmImage lap0,KpmImage lap1,KpmImage lap2,
                int x,int y) 
        {
...
//                int x,int y)
		...
		assert (x-1) >= 0 && (x+1) < lap1.getWidth();//		ASSERT((x-1) >= 0 && (x+1) < lap1.width(), "x out of bounds");
		assert (y-1) >= 0 && (y+1) < lap1.getHeight();//		ASSERT((y-1) >= 0 && (y+1) < lap1.height(), "y out of bounds");
		assert (lap0.getWidth()>>1) == lap1.getWidth();//		ASSERT((lap0.width()>>1) == lap1.width(), "Image dimensions inconsistent");
		assert (lap0.getWidth()>>1) == lap2.getWidth();//		ASSERT((lap0.width()>>1) == lap2.width(), "Image dimensions inconsistent");
		assert (lap0.getHeight()>>1) == lap1.getHeight();//		ASSERT((lap0.height()>>1) == lap1.height(), "Image dimensions inconsistent");
		assert (lap0.getHeight()>>1) == lap2.getHeight();//		ASSERT((lap0.height()>>1) == lap2.height(), "Image dimensions inconsistent");
...
//		ComputeSubpixelDerivatives(Dx, Dy, Dxx, Dyy, Dxy, lap1, x, y);
		...
		Dxs = 0.25f*((bilinear_interpolation(lap0, x_mul_2-2, y_mul_2) + lap2buf[lap2_p+1]) -
		(bilinear_interpolation(lap0, x_mul_2+2, y_mul_2) + lap2buf[lap2_p-1]));
		Dys = 0.25f*((bilinear_interpolation(lap0, x_mul_2, y_mul_2-2) + lap2buf[lap2_pp1+0]) -
		(bilinear_interpolation(lap0, x_mul_2, y_mul_2+2) + lap2buf[lap2_pm1+0]));
...
//        const Image& lap0,const Image& lap1,const Image& lap2,
//        int x,int y)        
	private void ComputeSubpixelHessianFineOctavePair(
		float[] H,float[] b,
	    KpmImage lap0,KpmImage lap1,KpmImage lap2,
	    int x,int y)
	{
		...
		assert (x-1) >= 0 && (x+1) < lap1.getWidth();//ASSERT((x-1) >= 0 && (x+1) < lap1.width(), "x out of bounds");
		assert (y-1) >= 0 && (y+1) < lap1.getHeight();//ASSERT((y-1) >= 0 && (y+1) < lap1.height(), "y out of bounds");
		assert lap0.getWidth() == lap1.getWidth();//ASSERT(lap0.width() == lap1.width(), "Image dimensions inconsistent");
		assert (lap0.getWidth()>>1) == lap2.getWidth();//ASSERT((lap0.width()>>1) == lap2.width(), "Image dimensions inconsistent");
		assert lap0.getHeight() == lap1.getHeight();//ASSERT(lap0.height() == lap1.height(), "Image dimensions inconsistent");
		assert (lap0.getHeight()>>1) == lap2.getHeight();//ASSERT((lap0.height()>>1) == lap2.height(), "Image dimensions inconsistent");
	
		int lap0_pm1 = lap0.get(y-1)+x;//const float* lap0_pm1 = &lap0.get<float>(y-1)[x];
		int lap0_p=lap0.get(y)+x;//const float* lap0_p   = &lap0.get<float>(y)[x];
		int lap0_pp1 =lap0.get(y+1)+x;//const float* lap0_pp1 = &lap0.get<float>(y+1)[x];
		int lap1_p   =lap1.get(y)+x;//const float* lap1_p   = &lap1.get<float>(y)[x];
		float[] tmp=new float[5];
		bilinear_downsample_point(tmp, x, y, 1);
		x_div_2=tmp[0];
		y_div_2=tmp[0];
		assert x_div_2-0.5f >= 0;//ASSERT(x_div_2-0.5f >= 0, "x_div_2 out of bounds out of bounds for interpolation");
		assert y_div_2-0.5f >= 0;//ASSERT(y_div_2-0.5f >= 0, "y_div_2 out of bounds out of bounds for interpolation");
		assert x_div_2+0.5f < lap2.getWidth();//ASSERT(x_div_2+0.5f < lap2.width(), "x_div_2 out of bounds out of bounds for interpolation");
		assert y_div_2+0.5f < lap2.getHeight();//, "y_div_2 out of bounds out of bounds for interpolation");
...
//		ComputeSubpixelDerivatives(Dx, Dy, Dxx, Dyy, Dxy, lap1, x, y);
		...
		//Dxs = 0.25f*((lap0_p[-1]  + bilinear_interpolation(lap2, x_div_2+.5f, y_div_2)) -
		//(lap0_p[ 1]  + bilinear_interpolation(lap2, x_div_2-.5f, y_div_2)));
		//Dys = 0.25f*((lap0_pm1[0] + bilinear_interpolation(lap2, x_div_2,     y_div_2+.5f)) -
		...
		Dxs = 0.25f*((lap0_buf[lap0_p-1]  + bilinear_interpolation(lap2, x_div_2+.5f, y_div_2)) -
		(lap0_buf[lap0_p+1]  + bilinear_interpolation(lap2, x_div_2-.5f, y_div_2)));
		Dys = 0.25f*((lap0_buf[lap0_pm1+0] + bilinear_interpolation(lap2, x_div_2,     y_div_2+.5f)) -
		(lap0_buf[lap0_pp1+0] + bilinear_interpolation(lap2, x_div_2,     y_div_2-.5f)));
...
//                int x,int y)        
		...
		assert (y-1) >= 0 && (y+1) < lap1.getHeight();// ASSERT((y-1) >= 0 && (y+1) < lap1.height(), "y out of bounds");
		assert lap0.getWidth() == lap1.getWidth(); //ASSERT(lap0.width() == lap1.width(), "Image dimensions inconsistent");
		assert lap0.getWidth() == lap2.getWidth();//ASSERT(lap0.width() == lap2.width(), "Image dimensions inconsistent");
		assert lap0.getHeight() == lap1.getHeight();//ASSERT(lap0.height() == lap1.height(), "Image dimensions inconsistent");
		assert lap0.getHeight() == lap2.getHeight();//ASSERT(lap0.height() == lap2.height(), "Image dimensions inconsistent");
		...
		int lap1_p   = lap1.get(y)+x;//const float* lap1_p   = &lap1.get<float>(y)[x];
		int lap2_pm1 = lap2.get(y-1)+x;//		const float* lap2_pm1 = &lap2.get<float>(y-1)[x];
		int lap2_p   = lap2.get(y)+x;//		const float* lap2_p   = &lap2.get<float>(y)[x];
		int lap2_pp1 = lap2.get(y+1)+x;//		const float* lap2_pp1 = &lap2.get<float>(y+1)[x];
...
//		ComputeSubpixelDerivatives(Dx, Dy, Dxx, Dyy, Dxy, lap1, x, y);
        ...
        void PruneDoGFeatures(
    		BucketStack[][] buckets,
    		FeaturePointStack outPoints,
    		FeaturePointStack inPoints,
            int num_buckets_X,
            int num_buckets_Y,
            int width,
            int height,
