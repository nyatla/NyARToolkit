...
public class MatrixCodeGen9 extends MatTable{
	public MatrixCodeGen9(int i_size) {
		super(i_size);
	}
	/**
	 * ベースクラス
...
//			return t.src_section;
	...
	protected MatrixCodeGen9(int i_size,MatItem[][] t) {
		super(i_size,t);
	}
	...
	public Factor absStr(ElementMap map){
		if(this._size==2){
			...
			for(int i=0;i<this._size;i++){
				MatrixCodeGen9 c=new MatrixCodeGen9(this._size-1,this.getCofactor(0,i));
				ValueFactor v;
	...
	public String inversMatrix()
	{
		...
		for(int r=0;r<this._size;r++){
			for(int c=0;c<this._size;c++){
				...
				Factor val=new MatrixCodeGen9(this._size-1,this.getCofactor(r, c)).absStr(map);
				//名前は転値
...
/*		for(int r=0;r<8;r++){
		...
		*/
		...
		MatrixCodeGen9 m33=new MatrixCodeGen9(8);
