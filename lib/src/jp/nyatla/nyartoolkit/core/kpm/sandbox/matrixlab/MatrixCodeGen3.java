...
public class MatrixCodeGen3 extends MatTable{
	public MatrixCodeGen3(int i_size) {
		super(i_size);
	}
	public class Tag{
	...
	protected MatrixCodeGen3(int i_size,MatItem[][] t) {
		super(i_size,t);
	}
	public class Result{
		public int lv;
	...
	public String absStr(ElementMap map,int flag,Result ret){
		if(this._size==2){
			...
			for(int i=0;i<this._size;i++){
				MatrixCodeGen3 c=new MatrixCodeGen3(this._size-1,this.getCofactor(0,i));
...
//				s+=String.format("%s*(%s)",v1,v2);
	...
	public String inversMatrix()
	{
		...
		for(int r=0;r<this._size;r++){
			for(int c=0;c<this._size;c++){
				...
				String val=new MatrixCodeGen3(this._size-1,this.getCofactor(r, c)).absStr(map,f,ret);
				//名前は転値
	...
	public static void main(String[] args){
		MatrixCodeGen3 m33=new MatrixCodeGen3(8);
