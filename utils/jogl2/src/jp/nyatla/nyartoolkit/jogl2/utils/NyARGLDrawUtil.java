...
public class NyARGLDrawUtil
{
		...
		if (polyList == 0) {
			...
			for (f = 0; f < cube_num_faces; f++)
				for (i = 0; i < 4; i++) {
					...
					gl.glVertex3f(CUBE_VERTICES[CUBE_FACE[f][i]][0] * fSize, CUBE_VERTICES[CUBE_FACE[f][i]][1] * fSize, CUBE_VERTICES[CUBE_FACE[f][i]][2] * fSize);
				}
			gl.glEnd();
	...
	public static void drawBackGround(GL i_gl,INyARRaster i_raster, double i_zoom) throws NyARRuntimeException
	{
		...
		gl.glGetTexEnviv(GL2.GL_TEXTURE_ENV, GL2.GL_TEXTURE_ENV_MODE, texEnvModeSave); // Save GL texture environment mode.
		if (texEnvModeSave.array()[0] != GL.GL_REPLACE) {
			gl.glTexEnvi(GL2.GL_TEXTURE_ENV, GL2.GL_TEXTURE_ENV_MODE, GL.GL_REPLACE);
		}
		lightingSave = gl.glIsEnabled(GL2.GL_LIGHTING); // Save enabled state of lighting.
		...
		gl.glPushMatrix();
		gl.glLoadIdentity();
		gl.glOrtho(0.0,rsize.w, 0.0,rsize.h,0,1);
		...
		if (lightingSave) {
			gl.glEnable(GL2.GL_LIGHTING); // Restore enabled state of lighting.
		}
		if (texEnvModeSave.get(0) != GL.GL_REPLACE) {
			gl.glTexEnvi(GL2.GL_TEXTURE_ENV, GL2.GL_TEXTURE_ENV_MODE, texEnvModeSave.get(0)); // Restore GL texture environment mode.
		}
		gl.glEnd();
	}
	/**
	 ...
	 * @param zoom
	 */
	private static void arglDispImageStateful(GL i_gl,NyARIntSize i_size,Object i_buffer,int i_buffer_type, double zoom) throws NyARRuntimeException
	{
		GL2 gl=i_gl.getGL2();
