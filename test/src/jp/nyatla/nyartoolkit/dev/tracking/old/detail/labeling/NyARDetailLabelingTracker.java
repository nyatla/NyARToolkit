...
public class NyARDetailLabelingTracker
{
	...
	public boolean addTrackTarget(NyAROutlineTrackItem i_item,double i_width,int i_direction) throws NyARException
	{
		...
		temp_linear[0].makeLinearWithNormalize(vtx_ptr[0],vtx_ptr[1]);
		temp_linear[1].makeLinearWithNormalize(vtx_ptr[1],vtx_ptr[2]);
		temp_linear[2].makeLinearWithNormalize(vtx_ptr[2],vtx_ptr[3]);
		temp_linear[3].makeLinearWithNormalize(vtx_ptr[3],vtx_ptr[0]);
		//3次元位置を計算
