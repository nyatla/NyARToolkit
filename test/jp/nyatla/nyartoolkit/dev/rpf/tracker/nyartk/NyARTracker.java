...
public class NyARTracker
{
	...
	public NyARTracker() throws NyARException
	{
		...
		this._map=new DistMap(20,20);
		this._index=new int[20];
	...
	/**
	 * アップグレードパラメータ。1処理でContoureへ遷移させるターゲットの最大値です。
	 */
	private static int UPGPARAM_NEW_AUTO_UPGREDE_PER_FRAME=1;
	/**
	 ...
	 * @throws NyARException
	 */
	private void upgradeNewTarget(NyARGrayscaleRaster i_base_raster,NyARTrackerOut i_trackdata) throws NyARException
	{
...
System.out.println("drop:new->ignore"+t.serial+":"+t.last_update);
		...
		len_of_new=i_trackdata.newtarget.getLength();
		//アップグレード処理の回数を計算
		int lp=len_of_new<UPGPARAM_NEW_AUTO_UPGREDE_PER_FRAME?len_of_new:UPGPARAM_NEW_AUTO_UPGREDE_PER_FRAME;
		...
		for(int i=lp-1;i>=0;i--)
		{
			...
			//ステータスの値をセット
			if(!c.setValue(i_base_raster,st.sampleout)){
...
System.out.println("lost:ignore:"+t.serial+":"+t.last_update);
	 ...
	 * @throws NyARException
	 */
	public void updateContureStatus(NyARGrayscaleRaster i_base_raster,NyARTrackerOut i_trackdata) throws NyARException
	{
		long clock=this._clock;
		...
		for(int i=len_of_crd-1;i>=0;i--){
			...
			if(!st.setValue(i_base_raster,s)){
				//新しいステータスのセットに失敗？
				st.releaseObject();
				continue;
			}			
	...
	public void updateRectStatus(NyARGrayscaleRaster i_base_raster,NyARTrackerOut i_trackdata) throws NyARException
	{
		long clock=this._clock;
		...
		for(int i=len_of_rct-1;i>=0;i--){
			...
			//ステータスを更新
			if(!st.setValue(i_trackdata.contourst_pool,i_base_raster,s)){
				//新しいステータスのセットに失敗？
				st.releaseObject();
				continue;
			}			
