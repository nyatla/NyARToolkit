...
import jp.nyatla.nyartoolkit.dev.tracking.square.*;
...
public class MarkerTracking_3dTrans
{
	...
	private class DetectSquareCB implements NyARSquareContourDetector.IDetectMarkerCallback
	{
			...
			double i_marker_width) throws NyARException
		{
			...
			this._outline_tracker=new NyAROutlineTracker(10,10);

			this._detail_table=new NyARDetailTrackSrcTable(10);
			this._detail_tracker=new NyARDetailTracker(10,10);
		...
		public NyAROutlineTracker _outline_tracker;
		public NyARDetailTracker _detail_tracker;
		...
		public void onSquareDetect(NyARSquareContourDetector i_sender,NyARIntPoint2d[] i_coord,int i_coorx_num,int[] i_vertex_index) throws NyARException
		{
			...
			if(this._detail_tracker.isTrackTarget(item.center))
			{
				this._detail_table.push(item,i_coord,i_coorx_num,i_vertex_index);
				return;
			}
			
			//トラッキング対象でなければ、追加
			if(!this._outline_tracker.isTrackTarget(item.center))
			{
		 ...
		 * コールバックハンドラの終期化
		 */
		public final void finish() throws NyARException
		{
			//ディティールトラッキングを実行
			...
			for(int i=this._new_outline_table.getLength()-1;i>=0;i--){
				this._outline_tracker.addTrackTarget(this._new_outline_table.getItem(i));
			}
		}
	}
	/**
	 * 指定したIDの対象を、detailTrackingにアップグレードする。
	 * この関数は、リスナ関数の中では使えない。
	 */
	public boolean upgradeToDetail(int i_serial,double i_width,int i_direction) throws NyARException
	{
		NyAROutlineTrackItem item=this._detect_cb._outline_tracker.getOutlineBySerial(i_serial);
		if(item==null){
			return false;
		}
		if(!this._detect_cb._detail_tracker.addTrackTarget(item, i_width, i_direction)){
			return false;
		}
		return true;
	...
	public Object[] _probe()
	{
		...
		ret[0]=this._detect_cb._outline_tracker;
