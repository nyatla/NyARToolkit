...
public class NyARTransMat_O1 implements INyARTransMat
{
	 ...
	 * @throws NyARException
	 */
	private final double arGetTransMat3(double ppos2d[][], final double i_pos3d[][], final double i_off[], NyARTransMatResult o_result_conv) throws NyARException
	{
	...
	private final double arGetTransMatSub(double i_ppos2d[][], double i_pos3d[][], double[] i_off, NyARTransMatResult o_result_conv) throws NyARException
	{
		...
		for (i = 0; i < NUMBER_OF_VERTEX; i++) {
			...
			a_array[x2 + 1][2] = b_array[2][x2 + 1] = cpara[1 * 4 + 2] - po2d_pt[1];// mat_a->m[j*6+5]
																					...
																					// -
																					// pos2d[j][1];
			c_array[x2][0] = wz * po2d_pt[0] - cpara[0 * 4 + 0] * wx - cpara[0 * 4 + 1] * wy - cpara[0 * 4 + 2] * wz;// mat_c->m[j*2+0]
																														// = wz
																														// *
																														// pos2d[j][0]-
																														// cpara[0][0]*wx
