...
abstract class NyARTransRot_OptimizeCommon implements NyARTransRot
{
	...
	/**
	 * static int check_dir( double dir[3], double st[2], double ed[2],double
	 * cpara[3][4] ) Optimize:STEP[526->468]
	 * 
	 * @param dir
	 * @param st
	 * @param ed
	...
	protected final void check_dir(double dir[], double st[], double ed[], double cpara[]) throws NyARException
	{
		...
		// <Optimize>
		// world[0][0] = a_array[0][0]*st[0]*10.0+ a_array[0][1]*st[1]*10.0+
		// a_array[0][2]*10.0;//mat_a->m[0]*st[0]*10.0+ mat_a->m[1]*st[1]*10.0+
		// mat_a->m[2]*10.0;
		// world[0][1] = a_array[1][0]*st[0]*10.0+ a_array[1][1]*st[1]*10.0+
		// a_array[1][2]*10.0;//mat_a->m[3]*st[0]*10.0+ mat_a->m[4]*st[1]*10.0+
		// mat_a->m[5]*10.0;
		// world[0][2] = a_array[2][0]*st[0]*10.0+ a_array[2][1]*st[1]*10.0+
		// a_array[2][2]*10.0;//mat_a->m[6]*st[0]*10.0+ mat_a->m[7]*st[1]*10.0+
		// mat_a->m[8]*10.0;
		// world[1][0] = world[0][0] + dir[0];
		// world[1][1] = world[0][1] + dir[1];
		// world[1][2] = world[0][2] + dir[2];
		world[0] = a_array[0][0] * st[0] * 10.0 + a_array[0][1] * st[1] * 10.0 + a_array[0][2] * 10.0;// mat_a->m[0]*st[0]*10.0+
																										// mat_a->m[1]*st[1]*10.0+
																										// mat_a->m[2]*10.0;
		world[1] = a_array[1][0] * st[0] * 10.0 + a_array[1][1] * st[1] * 10.0 + a_array[1][2] * 10.0;// mat_a->m[3]*st[0]*10.0+
																										// mat_a->m[4]*st[1]*10.0+
																										// mat_a->m[5]*10.0;
		world[2] = a_array[2][0] * st[0] * 10.0 + a_array[2][1] * st[1] * 10.0 + a_array[2][2] * 10.0;// mat_a->m[6]*st[0]*10.0+
																										// mat_a->m[7]*st[1]*10.0+
																										// mat_a->m[8]*10.0;
	...
	/**
	 * パラメタa,b,cからrotを計算してインスタンスに保存する。 rotを1次元配列に変更
...
class NyARTransRot_O1 extends NyARTransRot_OptimizeCommon
{
	...
	/**
	 * int arGetInitRot( ARMarkerInfo *marker_info, double cpara[3][4], double
	 * rot[3][3] ) Optimize:2008.04.20:STEP[716→698]
	 * 
	 * @param marker_info
	 * @param i_direction
	 * @param i_param
	...
	public final void initRot(NyARSquare marker_info, int i_direction) throws NyARException
	{
		...
		wdir[2][2] /= w;
		/*
		 * if( wdir[2][2] < 0 ) { wdir[2][0] /= -w; wdir[2][1] /= -w; wdir[2][2] /=
		 * -w; } else { wdir[2][0] /= w; wdir[2][1] /= w; wdir[2][2] /= w; }
		 */
		// <Optimize>
		// rot[0][0] = wdir[0][0];
		// rot[1][0] = wdir[0][1];
...
/**
 * NyARModifyMatrixの最適化バージョン2 計算手順の変更、構造変更など含む最適化をしたもの
 * 
 */
class NyARTransRot_O2 extends NyARTransRot_OptimizeCommon
{
	...
	public double modifyMatrix(double trans[], double vertex[][], double pos2d[][]) throws NyARException
	{
		...
		int s1 = 0, s2 = 0, s3 = 0;
		int i, j;
		final double[] combo = this.wk_arModifyMatrix_combo;// arGetNewMatrixで初期化されるので初期化不要//new
															// double[3][4];
			...
			for (t1 = -1; t1 <= 1; t1++) {
				...
				for (t2 = -1; t2 <= 1; t2++) {
					...
					for (t3 = -1; t3 <= 1; t3++) {
						...
						arGetRotC(c1, rot);
						// comboの0-2行目を計算
						nyatla_arGetNewMatrix_row012(rot, trans, combo);// 第二パラメタは常にnull//arGetNewMatrix(trans,
																		// null,
																		// combo
																		// );
						err = 0.0;
	...
	/**
	 * arGetNewMatrixの0-2行目を初期化する関数 Optimize:2008.04.20:STEP[569->144]
	 * 
	 * @param i_rot
	 * @param trans
	 * @param trans2
