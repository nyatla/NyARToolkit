...
/**
 * イメージからマーカー情報を検出するクラス。 このクラスは、arDetectMarker2.cとの置き換えになります。
...
public class NyARDetectMarker
{
	...
	/**
	 * int arGetContour( ARInt16 *limage, int *label_ref,int label, int clip[4],
	 * ARMarkerInfo2 *marker_info2 ) 関数の代替品
	 * detectMarker関数から使う関数です。o_markerにlabelとclipで示される1個のマーカーを格納します。
	 * marker_holder[i_holder_num]にオブジェクトが無ければまず新規に作成し、もし
	 * 既に存在すればそこにマーカー情報を上書きして記録します。 Optimize:STEP[369->336]
	 * 
	 * @param o_marker
	 * @param limage
	 * @param label_ref
	...
	private final void arGetContour(NyARMarker o_marker, int[][] limage, int i_labelnum, NyARLabelingLabel i_label) throws NyARException
	{
		...
		for (i = i_label.clip_l; i <= clip1; i++) {// for( i = clip[0]; i <=
			// clip[1]; i++, p1++ ) {
			w = limage_j[i];
			if (w > 0 && w == i_labelnum) {
				sx = i;
		...
		for (;;) {
			...
			// p1 = &(limage[marker_info2->y_coord[marker_info2->coord_num-1] *
			// xsize+ marker_info2->x_coord[marker_info2->coord_num-1]]);
			dir = (dir + 5) % 8;
			for (i = 0; i < 8; i++) {
				if (limage[r + ydir[dir]][c + xdir[dir]] > 0) {// if(
					// p1[ydir[dir]*xsize+xdir[dir]]
					// > 0 ){
					break;
				}
				dir = (dir + 1) % 8;
			...
			c = c + xdir[dir];// marker_info2->x_coord[marker_info2->coord_num]=
			// marker_info2->x_coord[marker_info2->coord_num-1]
			// + xdir[dir];
			r = r + ydir[dir];// marker_info2->y_coord[marker_info2->coord_num]=
			// marker_info2->y_coord[marker_info2->coord_num-1]
			// + ydir[dir];
		...
		// d = (xcoord[i]-sx)*(xcoord[i]-sx)+ (ycoord[i]-sy)*(ycoord[i]-sy);
		// d = (marker_info2->x_coord[i]-sx)*(marker_info2->x_coord[i]-sx)+(marker_info2->y_coord[i]-sy)*(marker_info2->y_coord[i]-sy);
		// if( d > dmax ) {
		// dmax = d;
		// v1 = i;
		// }
	...
	/**
	 * ARMarkerInfo2 *arDetectMarker2( ARInt16 *limage, int label_num, int
	 * *label_ref,int *warea, double *wpos, int *wclip,int area_max, int
	 * area_min, double factor, int *marker_num ) 関数の代替品
	...
	public final void detectMarker(NyARLabelingImage i_labeling, double i_factor, NyARMarkerList o_marker_list) throws NyARException
	{
		...
		int i;
		int xsize, ysize;
		NyARLabelingLabel[] labels = i_labeling.getLabelStack().getArray();
		// int[] warea =i_labeling.getArea();
		int label_num = i_labeling.getLabelStack().getLength();
		int[][] limage = i_labeling.getBufferObject();
		...
		for (i = 0; i < label_num; i++) {
			...
			// ret = arGetContour( limage, label_ref, i+1,&(wclip[i*4]),
			// &(marker_info2[marker_num2]));
