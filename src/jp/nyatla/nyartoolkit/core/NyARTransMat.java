...
public class NyARTransMat{
	...
	for(int i = 0; i < NUMBER_OF_VERTEX; i++ ) {
	    o_pos3d[i*3+0] = i_ppos3d[i][0] + o_off[0];
	    o_pos3d[i*3+1] = i_ppos3d[i][1] + o_off[1];
	    o_pos3d[i*3+2] = 0.0;
	}
    }    
...
//	mat_b.realloc(3,NUMBER_OF_VERTEX*2);
	...
	if(arFittingMode == AR_FITTING_TO_INPUT ){
		...
		pos2d[i*2+0]=a1.value;
		pos2d[i*2+1]=a2.value;
	    }
	}else{
	    for( i = 0; i < NUMBER_OF_VERTEX; i++ ) {
		pos2d[i*2+0] = i_ppos2d[i][0];
		pos2d[i*2+1] = i_ppos2d[i][1];
	    }
	}
...
//	mat_c.realloc(NUMBER_OF_VERTEX*2,1);
	...
	for( j = 0; j < NUMBER_OF_VERTEX; j++ ) {
	    ...
	    wx = rot[0] * i_pos3d[j*3+0]+ rot[1] * i_pos3d[j*3+1]+ rot[2] * i_pos3d[j*3+2];
	    wy = rot[3] * i_pos3d[j*3+0]+ rot[4] * i_pos3d[j*3+1]+ rot[5] * i_pos3d[j*3+2];
	    wz = rot[6] * i_pos3d[j*3+0]+ rot[7] * i_pos3d[j*3+1]+ rot[8] * i_pos3d[j*3+2];
	    //</Optimize>
	    ...
	    a_array[x2  ][2]=b_array[2][x2]=cpara[0*4+2] - pos2d[j*2+0];//mat_a->m[j*6+2] = mat_b->m[num*4+j*2] = cpara[0][2] - pos2d[j][0];
	    ...
	    a_array[x2+1][2]=b_array[2][x2+1]=cpara[1*4+2] - pos2d[j*2+1];//mat_a->m[j*6+5] = mat_b->m[num*4+j*2+1] = cpara[1][2] - pos2d[j][1];
	    c_array[x2][0]  =wz * pos2d[j*2+0]- cpara[0*4+0]*wx - cpara[0*4+1]*wy - cpara[0*4+2]*wz;//mat_c->m[j*2+0] = wz * pos2d[j][0]- cpara[0][0]*wx - cpara[0][1]*wy - cpara[0][2]*wz;
	    c_array[x2+1][0]=wz * pos2d[j*2+1]- cpara[1*4+1]*wy - cpara[1*4+2]*wz;//mat_c->m[j*2+1] = wz * pos2d[j][1]- cpara[1][1]*wy - cpara[1][2]*wz;
	}
...
//	}
	...
	ret = transrot.modifyMatrix(trans, i_pos3d, pos2d);
	double[][] conv=result_mat.getArray();
	for( j = 2; j >=0; j-- ) {//<Optimize/>for( j = 0; j < 3; j++ ) {
	    //<Optimize>
	    //for( i = 0; i < 3; i++ ){
	    //	conv[j][i] = rot[j][i];
	    //}
