...
public class NyARSingleDetectMarker
{
	...
	// INyARRasterReaderFactory _reader_factory=new
	// NyARRasterReaderFactory_RgbTotal();
	NyARBinRaster _bin_raster = null;
	INyARRasterFilter _tobin_filter=new NyARRasterFilter_ARToolkitThreshold(100);
	 ...
	 * @throws NyARException
	 */
	public boolean detectMarkerLite(INyARRgbRaster i_raster, int i_thresh) throws NyARException
	{
		//【バグ】i_thresh無視してる
		//【バグ】ラスタサイズの変更追跡できてない。
		if(this._bin_raster==null){
			this._bin_raster=new NyARBinRaster(i_raster.getWidth(),i_raster.getHeight());
		}
		//ラスタを２値イメージに変換する。
		...
		// スクエアコードを探す
		square.detectSquare(this._bin_raster, l_square_list);
		...
		if (!patt.pickFromRaster(i_raster, l_square_list.getSquare(0))) {
			// パターンの切り出しに失敗
			return false;
		}
		// パターンを評価器にセット
		...
		for (int i = 1; i < number_of_square; i++) {
			// 次のパターンを取得
			patt.pickFromRaster(i_raster, l_square_list.getSquare(i));
			// 評価器にセットする。
